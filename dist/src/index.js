...
import { validateName, validateAgentName, validateCwd, validateRegex, validatePermissionMode, validateEffort, validateTimeout, validatePositiveInt, validateStringField, validateStringArray, validateBoolean, } from './validation.js';
// ─── Plugin ───────────────────────────────────────────────────────────────────
const plugin = {
    id: 'clawnductor',
    name: 'Clawnductor',
    description: 'Weaponize Claude Code from OpenClaw — persistent sessions, multi-agent ensembles, ultraplan, and fleet review.',
    register(api) {
                ...
                execute: async (id, args) => {
                    ...
                    try {
                        ...
                        if (result && typeof result === 'object' && 'ok' in result && !result.ok) {
                            api.logger.error(`[tool:${t.name}] returned ok:false —`, result.error ?? result);
                        }
                        return result;
                    }
                    catch (err) {
        ...
        registerTool({
            name: 'jam_start',
            description: 'Start a persistent Claude Code session (a "jam"). Returns a session name you pass to jam_play, jam_bridge, etc. Sessions stay alive across requests — use jam_list to see active ones.',
            parameters: {
                type: 'object',
                properties: {
            ...
            execute: async (_id, args) => {
                ...
                if (args.name !== undefined)
                    input.name = validateName(args.name, 'name');
                if (args.cwd !== undefined)
                    input.cwd = validateCwd(args.cwd, 'cwd');
                if (args.model !== undefined)
                    input.model = validateStringField(args.model, 'model', 100);
                if (args.permissionMode !== undefined)
                    input.permissionMode = validatePermissionMode(args.permissionMode, 'permissionMode');
                if (args.effort !== undefined)
                    input.effort = validateEffort(args.effort, 'effort');
                if (args.allowedTools !== undefined)
                    input.allowedTools = validateStringArray(args.allowedTools, 'allowedTools');
                if (args.disallowedTools !== undefined)
                    input.disallowedTools = validateStringArray(args.disallowedTools, 'disallowedTools');
                if (args.maxTurns !== undefined)
                    input.maxTurns = validatePositiveInt(args.maxTurns, 'maxTurns', 500);
                if (args.appendSystemPrompt !== undefined)
                    input.appendSystemPrompt = validateStringField(args.appendSystemPrompt, 'appendSystemPrompt');
                if (args.bare !== undefined)
                    input.bare = validateBoolean(args.bare, 'bare');
                if (args.resumeSessionId !== undefined)
                    input.resumeSessionId = validateStringField(args.resumeSessionId, 'resumeSessionId', 200);
                if (args.forkSession !== undefined)
                    input.forkSession = validateBoolean(args.forkSession, 'forkSession');
                if (args.noSessionPersistence !== undefined)
                    input.noSessionPersistence = validateBoolean(args.noSessionPersistence, 'noSessionPersistence');
        ...
        registerTool({
            ...
            execute: async (_id, args) => {
                ...
                if (args.nowait) {
                    ...
                    if (!s.stats.isReady)
                        throw new Error(`Session "${name}" is not ready`);
                    if (s.stats.busy)
                        throw new Error(`Session "${name}" is busy`);
                    getManager().sendMessage(name, message, sendOpts).catch(() => { });
                    return { ok: true, pending: true, name };
                }
                const chunks = [];
                const result = await getManager().sendMessage(name, message, { ...sendOpts, onChunk: args.stream ? (c) => chunks.push(c) : undefined });
        ...
        registerTool({
            name: 'bandstand',
            description: 'Health overview of every active jam session: readiness, busy/paused state, cost, context %. Use this for a dashboard view; use jam_status for single-session detail.',
            parameters: { type: 'object', properties: {} },
            execute: async () => {
                if (!manager)
                    return { ok: true, sessions: 0, sessionNames: [], details: [] };
        ...
        registerTool({
            ...
            execute: async (_id, args) => {
                const matches = await getManager().grepSession(validateName(args.name, 'name'), validateRegex(args.pattern, 'pattern'), args.limit !== undefined ? validatePositiveInt(args.limit, 'limit', 500) : undefined);
                return { ok: true, count: matches.length, matches };
            },
        });
        registerTool({
            name: 'jam_bridge',
            description: 'Compact a session\'s context to reclaim context window space. Like a musical bridge — connects what came before to what comes next.',
            parameters: {
                type: 'object',
                properties: {
                    name: { type: 'string' },
        ...
        registerTool({
            name: 'jam_transpose',
            description: 'Switch the model for a running session. Restarts subprocess with --resume to preserve history. Like transposing a song to a new key.',
            parameters: {
                type: 'object',
                properties: {
                    name: { type: 'string' },
            ...
            execute: async (_id, args) => {
                const info = await getManager().switchModel(validateName(args.name, 'name'), validateStringField(args.model, 'model', 100));
                return { ok: true, restarted: true, ...info };
            },
        });
        registerTool({
            name: 'jam_rekey',
            description: 'Update allowed/disallowed tools for a running session. Restarts with --resume. Like changing the key signature mid-score.',
            parameters: {
                type: 'object',
                properties: {
                    name: { type: 'string' },
        ...
        registerTool({
            name: 'ensemble_start',
            description: 'Start a multi-agent ensemble on a coding task. Agents work in parallel git worktrees, vote on completion, and merge to main. Returns an ensemble ID — poll with ensemble_status.',
            parameters: {
                type: 'object',
                properties: {
            ...
            execute: async (_id, args) => {
                ...
                agents = agents.map((a) => ({
                    ...
                    ...(a.permissionMode !== undefined ? { permissionMode: validatePermissionMode(a.permissionMode, 'agent.permissionMode') } : {}),
                }));
                const ensemble = getManager().ensembleStart(task, {
                    ...
                    defaultPermissionMode: args.defaultPermissionMode !== undefined
                        ? validatePermissionMode(args.defaultPermissionMode, 'defaultPermissionMode')
                        : 'bypassPermissions',
                });
        ...
        registerTool({
            ...
            execute: async (_id, args) => {
                ...
                if (!ensemble)
                    return { ok: false, error: 'Ensemble not found' };
        ...
        registerTool({
            ...
            execute: async (_id, args) => {
                getManager().ensembleInject(validateStringField(args.id, 'id', 36), validateStringField(args.message, 'message'));
                return { ok: true };
            },
        });
        registerTool({
            name: 'ensemble_score',
            description: 'Review completed ensemble output: changed files, branches, worktrees, plan.md, and per-agent summaries. Call before ensemble_accept or ensemble_reject.',
            parameters: {
                type: 'object',
                properties: { id: { type: 'string' } },
                required: ['id'],
        ...
        registerTool({
            ...
            execute: async (_id, args) => {
                const result = await getManager().ensembleReject(validateStringField(args.id, 'id', 36), validateStringField(args.feedback, 'feedback'));
                return { ok: true, ...result };
            },
        });
        // ═══════════════════════════════════════════════════════════════════════════
        ...
        registerTool({
            name: 'overture_start',
            description: 'Start a deep planning session — an Opus agent that explores your codebase thoroughly and produces a detailed implementation plan. Runs up to 30 minutes in background. Poll with overture_status.',
            parameters: {
                type: 'object',
                properties: {
        ...
        registerTool({
            ...
            execute: async (_id, args) => {
                ...
                if (!result)
                    return { ok: false, error: 'Overture not found' };
                return { ok: true, ...result };
            },
        });
        // ═══════════════════════════════════════════════════════════════════════════
        ...
        registerTool({
            name: 'finale_start',
            description: 'Launch a fleet of specialized code review agents in parallel — security, logic, performance, API, types, concurrency, and more. Runs in background. Poll with finale_status.',
            parameters: {
                type: 'object',
                properties: {
        ...
        registerTool({
            ...
            execute: async (_id, args) => {
                ...
                if (!result)
                    return { ok: false, error: 'Finale not found' };
                return { ok: true, ...result };
            },
        });
        // ═══════════════════════════════════════════════════════════════════════════
        ...
        registerTool({
            name: 'purge_stage',
            description: 'Wipe Claude Code project state (transcripts, tasks, file history) via `claude project purge`. Defaults to dry-run — pass dry_run: false to actually delete.',
            parameters: {
                type: 'object',
                properties: {
