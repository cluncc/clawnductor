...
class CircuitBreaker {
  ...
  isOpen(): boolean {
    ...
    if (this.openUntil && Date.now() >= this.openUntil) this.openUntil = 0;
    return false;
  }
  ...
  recordFailure(): void {
    ...
    if (this.failures >= this.threshold) {
      const backoff = Math.min(
        this.baseMs * Math.pow(2, this.failures - this.threshold),
        this.maxMs,
      );
...
describe('CircuitBreaker', () => {
  ...
  it('doubles backoff on consecutive failures past threshold', () => {
    ...
    cb1.recordFailure(); cb1.recordFailure(); cb1.recordFailure();
    // cb2: 4 failures → openUntil = now + 2000
    cb2.recordFailure(); cb2.recordFailure(); cb2.recordFailure(); cb2.recordFailure();
